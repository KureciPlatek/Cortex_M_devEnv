...
int main(void)
{
   MPU_Config();           /* Enable and set up the MPU (Memory Protection Unit) */

   SCB_EnableICache();     /* Enable CPU I-Cache */
   SCB_EnableDCache();     /* Enable CPU D-Cache */

   /* HAL init and setup systick to 1ms timebase */
   HAL_Init();             /* stm32n6xx_hal.c, do also HAL_MspInit() */

   SystemClock_Config();   /* Configure the system clock */
   GPIO_Init();            /* Configure GPIO pins */
   USART1_Init();          /* Configure UART */
   ThreadX_Init();         /* Configure and launch ThreadX*/
